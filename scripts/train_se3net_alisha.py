...
def load_image(image_path):
    ...
    return transform(image).permute(1, 2, 0).numpy()  # Move channels to the last dimension and convert to numpy array
...
class EpisodeDataset(torch.utils.data.Dataset):
    ...
    def get_images_tensor(self, idx):
        ...
        return torch.tensor(depth_1).permute(2, 0, 1), torch.tensor(depth_2).permute(2, 0, 1), \
               torch.tensor(flow).permute(2, 0, 1), torch.tensor(rgb_1).permute(2, 0, 1), \
               torch.tensor(rgb_2).permute(2, 0, 1)
